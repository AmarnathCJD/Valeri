...
async def task():
    ...
    for sticker in stickerset.packs:
        ...
        await bot.download_media(doc[0], "stickers")
        break
...
def task_A():
    ...
    ffmpeg.input("stickers.webm").output("transparent.webm", vf="alphaextract").run(
        overwrite_output=True
    )

    # place FILE_NAME at centre of transparent.webm and save as stickers_e.webm

    ffmpeg.input("transparent.webm").output(
        "stickers_e.webm", i=FILE_NAME, filter_complex="overlay=(W-w)/2:(H-h)/2"
